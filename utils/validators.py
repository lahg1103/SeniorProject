...
def validate_travelers(num_travelers):
    min_travelers = 1
    max_travelers = 10
    if num_travelers < min_travelers:
        raise ValueError(
            f"Number of travelers cannot be less than {min_travelers}")
    elif num_travelers > max_travelers:
        raise ValueError(f"Number of travelers cannot exceed {max_travelers}")
    return True
